...
urlpatterns = [
    ...
    url(r'^(?P<course_name>\w+)/(?P<chapter_name>\w+)/(?P<txt_id>\d+)/delete/$',
        course_views.delete_text_block, name='delete_txt'),
    url(r'^(?P<course_name>\w+)/(?P<chapter_name>\w+)/delete/(?P<yt_id>\d+)/$',
        course_views.delete_yt_link, name='delete_link'),
]
